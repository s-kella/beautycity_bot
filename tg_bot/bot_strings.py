# Main menu
welcome_msg = 'Привет! Добро пожаловать в бот BeautyCity!'
main_menu_msg = ('Главное меню. Запишитесь на услугу или зайдите в личный кабинет, '
                 'чтобы просмотреть существующие записи.')
account_menu_msg = 'Личный кабинет'

# New appointment
new_appointment_msg = 'Давайте запишем вас на услугу.'
salon_all_or_nearest = 'Хотите поделиться местоположением, чтобы увидеть ближайшие к вам салоны?'
request_location = 'Пришлите мне свое местоположение.'
choose_salon_menu = 'Выберите салон:'
choose_provider_menu = 'Выберите мастера:'
choose_service_menu = 'На какую услугу вы хотите записаться?'
week_menu = ('У нас есть расписание вашего мастера на следующие 4 недели. '
             'На какую неделю вы хотели бы записаться?')
date_menu = 'На выбранной неделе ваш мастер доступен в следующие дни:'
hour_menu = 'Выберите удобное время:'
appt_confirmed_msg = 'Ваша запись подтверждена!'


# Registration
not_registered_msg = 'Для продолжения нужно зарегистрироваться.'
policy_msg = ('Для продолжения, пожалуйста, подтвердите свое согласие с нашей политикой обработки данных: '
              'https://containers-qube.cn/storage/uploads/soglashenie-polzovatelya.pdf.')
request_phone_msg = 'Пожалуйста, поделитесь номером телефона.'
share_contact = 'Поделиться контактом'
declined_contact = ('Жаль! Будем рады зарегистрировать вас, если передумаете. '
                    'А пока можете посмотреть на наши салоны и мастеров.')
confirm_name_msg = 'Ваше имя - {}?'
request_name_msg = 'Пришлите мне свои имя и фамилию.'
registration_successful = ('Спасибо за регистрацию! '
                           'Вы можете продолжить то, что делали выше, или перейти в главное меню.')

decline_contact = 'В другой раз'

# Account
future_appointments_msg = 'Текущие записи:'
past_appointments_msg = 'Прошлые записи:'
no_appointments = 'Нет записей.'

# Button labels
new_appt = 'Новая запись'
back_to_new_appt = 'Начать сначала'
account_menu = 'Личный кабинет'
future_appointments_label = 'Текущие записи'
past_appointments_label = 'Прошлые записи'
main_menu = 'Главное меню'
back_to_main = 'Обратно в главное меню'
registration = 'Зарегистрироваться'
policy_agree = 'Я даю согласие на обработку данных'
confirm = 'Подтвердить'
change = 'Изменить'
choose_salon = 'Выбор по салону'
nearest_salons = 'Ближайшие'
all_salons = 'Все'
send_location = 'Отправить местоположение'
decline_location = 'Не хочу. Покажите все.'
choose_provider = 'Выбор по мастеру'
choose_service = 'Выбор по услуге'
this_week = 'Эта неделя'
next_week = 'Следующая неделя'
week_after_2 = 'Через 2 недели'
week_after_3 = 'Через 3 недели'
back_to_week = 'Назад к выбору недели'
back_to_date = 'Назад к выбору даты'


# Misc
thanks = 'Спасибо!'
ok = 'Нет проблем!'
db_error_message = 'При выполнении запроса произошла ошибка. Пожалуйста, повторите операцию позже.'
